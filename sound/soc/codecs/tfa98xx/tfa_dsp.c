...
void tfa_irq_update(Tfa98xx_handle_t handle) {
	...
	if (!maxregs) {
		...
		for (i=0; i<maxregs; i++) {
			...
			tfa_reg_read(handle, (unsigned char)reg, &regs_cache_irq_ena[i]);
		}
	}
...
enum Tfa98xx_Error tfa_reg_read(Tfa98xx_handle_t handle, unsigned char subaddress, unsigned short *value)
{
	enum Tfa98xx_Error error;

	if(handles_local[handle].dev_ops.tfa_reg_read) {
		error = (*handles_local[handle].dev_ops.tfa_reg_read)(handle, subaddress, value);
		if(error != Tfa98xx_Error_Ok)
			error = (enum Tfa98xx_Error) (error + Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */
	} else {
