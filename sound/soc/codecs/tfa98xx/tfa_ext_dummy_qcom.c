...
/*     Create directory `/sys/kernel/debug/tfa_ext_dummy`.*/
    ...
    if (!dir) {
        // Abort module load.
        printk(KERN_ALERT "debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy\n");
        return -1;
    }
...
/* Create file `/sys/kernel/debug/tfa_ext_dummy/event`.*/
    ...
    if (!junk) {
        // Abort module load.
        printk(KERN_ALERT "debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy/event\n");
        return -1;
    }
...
*/
    ...
    if (!junk) {
        printk(KERN_ALERT "debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy/status\n");
        return -1;
    }
