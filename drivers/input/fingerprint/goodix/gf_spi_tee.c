...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	gf_debug(DEBUG_LOG,"gf_power_enable enable = %d, onoff =%d\n", enable, onoff);
	...
	if (onoff && enable) {
		...
		gf_debug(DEBUG_LOG,"gf_power_enable enable high reset\n");
#endif
	} else if (!onoff && !enable) {
		enable = 1;
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_power_low);
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_low);
		gf_debug(DEBUG_LOG,"gf_power_enable enable low reset_low\n");
	}
}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INIT:
		...
		if (gf_dev->notifier.notifier_call == NULL) {
...
static int gf_release(struct inode *inode, struct file *filp)
{
	...
	if (gf_dev->irq) {
		gf_dev->system_status = 0;
		gf_disable_irq(gf_dev);
		gf_debug(DEBUG_LOG, "[%s][%d] disable irq, system_status=%d", __func__, __LINE__, gf_dev->system_status);

		free_irq(gf_dev->irq, gf_dev);
		gf_dev->irq_count = 0;
		gf_dev->irq = 0;
		gf_debug(DEBUG_LOG, "[%s][%d] free irq irq_count = %d", __func__, __LINE__, gf_dev->irq_count);
	}
	...
	if (gf_dev->pinctrl_gpios) {
		...
		gf_debug(DEBUG_LOG, "[%s][%d] devm_pinctrl_put gpio", __func__, __LINE__);
	}
