...
static int parse_arg(const char *buf, size_t count)
{
	...
	memcpy(cmdline_param, buf, min_t(size_t, (size_t) MAX_ARG_LENGTH, count));
					     ...
					     struct device_attribute *attr, const char *buf, size_t count)
{
	...
	data = kmalloc(argc - 1, GFP_KERNEL);
	if (data == NULL) {
	...
	if (ret) {
		cts_err("Write firmware register addr: 0x%04x size: %d failed", addr, argc - 1);
		goto free_data;
	}
...
static ssize_t read_firmware_register_show(struct device *dev, struct device_attribute *attr, char *buf)
{
#define PRINT_ROW_SIZE          (16)
	...
	for (i = 0; i < size && count <= PAGE_SIZE; i += PRINT_ROW_SIZE) {
		size_t linelen = min_t(size_t, (size_t) remaining, (size_t) PRINT_ROW_SIZE);
		...
		hex_dump_to_buffer(data + i, linelen, PRINT_ROW_SIZE, 1, buf + count, PAGE_SIZE - count, true);
...
static DEVICE_ATTR(read_reg, S_IWUSR | S_IRUSR, read_firmware_register_show, read_firmware_register_store);


static ssize_t read_sram_show(struct device *dev, struct device_attribute *attr, char *buf)
{
#define PRINT_ROW_SIZE          (16)
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	...
	ret = cts_hw_reg_readsb(cts_dev, addr, data, (size_t) size);
	if (ret) {
		count = snprintf(buf, PAGE_SIZE,
			"Read firmware register from 0x%04x size %u failed %d\n", addr, size, ret);
		goto err_free_data;
...
/* echo addr size > read_reg */
static ssize_t read_sram_store(struct device *dev,
					    struct device_attribute *attr, const char *buf, size_t count)
{
	parse_arg(buf, count);
...
static DEVICE_ATTR(read_sram, S_IWUSR | S_IRUSR, read_sram_show, read_sram_store);

static ssize_t curr_firmware_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t rows_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t cols_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t res_x_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t res_y_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t esd_protection_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	int ret;
	u8 esd_protection;
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Read firmware ESD protection register failed %d\n", ret);
	}
...
static ssize_t monitor_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	int ret;
	u8 value;
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Read firmware monitor enable register failed %d\n", ret);
	}

	return snprintf(buf, PAGE_SIZE, "Monitor mode: %s\n", value & BIT(0) ? "Enable" : "Disable");
}

static ssize_t monitor_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	int ret;
	u8 value = 0, enable = 0;
	...
	cts_info("Write firmware monitor mode to '%c', %s", buf[0], value ? "Enable" : "Disable");
...
static ssize_t auto_compensate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	int ret;
	u8 value;
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Read auto compensate enable register failed %d\n", ret);
	}
...
static ssize_t driver_builtin_firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int i, count = 0;
	...
	for (i = 0; i < cts_get_num_driver_builtin_firmware(); i++) {
		const struct cts_firmware *firmware = cts_request_driver_builtin_firmware_by_index(i);

		if (firmware) {
			count += snprintf(buf + count, PAGE_SIZE - count,
					  "%-2d: hwid: %04x fwid: %04x size: %6zu desc: %s\n",
					  i, firmware->hwid, firmware->fwid, firmware->size, firmware->name);
		} else {
			count += snprintf(buf + count, PAGE_SIZE - count, "%-2d: INVALID\n", i);
		}
	}
					     ...
					     struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (isdigit(*argv[0])) {
		ret = kstrtouint(argv[0], 10, &index);
		if (ret)
			return ret;
	}
	...
	cts_info("Update driver builtin firmware '%s' to %s", argv[1], to_flash ? "flash" : "sram");
...
static ssize_t updating_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t flash_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	...
	if (cts_dev->flash == NULL) {
		...
		if (ret) {
			return snprintf(buf, PAGE_SIZE, "Prepare flash operation failed %d", ret);
		}
		...
		if (!program_mode) {
			...
			if (ret) {
				return snprintf(buf, PAGE_SIZE, "Enter normal mode failed %d", ret);
			}
		}
		...
		if (enabled) {
			ret = cts_start_device(cts_dev);
			if (ret) {
				return snprintf(buf, PAGE_SIZE, "Start device failed %d", ret);
			}
		}
...
static ssize_t read_flash_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	...
	cts_info("Read flash from 0x%06x size %u%s%s",
		 flash_addr, size, argc == 3 ? " to file " : "", argc == 3 ? argv[2] : "");
	...
	if (argc == 3) {
		...
		if (IS_ERR(file)) {
			count += snprintf(buf, PAGE_SIZE, "Open file '%s' failed %ld", argv[2], PTR_ERR(file));
			goto err_post_flash_operation;
		}

		ret = kernel_write(file, data, size, 0);
		if (ret != size) {
			count += snprintf(buf, PAGE_SIZE, "Write flash data to file '%s' failed %d", argv[2], ret);
		}

		ret = filp_close(file, NULL);
		if (ret) {
			count += snprintf(buf, PAGE_SIZE, "Close file '%s' failed %d", argv[2], ret);
		}
	} else {
#define PRINT_ROW_SIZE          (16)
		remaining = size;
...
static ssize_t read_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	parse_arg(buf, count);
...
static ssize_t erase_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	u32 flash_addr, size;
	...
	if (ret) {
		cts_err("Erase flash from 0x%06x size %u failed %d", flash_addr, size, ret);
		goto err_post_flash_operation;
	}
...
static ssize_t open_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	u16 threshold;
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Open test FAILED %d, threshold = %u\n", ret, threshold);
	} else {
		return snprintf(buf, PAGE_SIZE, "Open test PASSED, threshold = %u\n", threshold);
	}
}

/* echo threshod > open_test */
static ssize_t open_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	parse_arg(buf, count);
...
static ssize_t short_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	u16 threshold;
	...
	if (ret) {
		return snprintf(buf, PAGE_SIZE, "Short test FAILED %d, threshold = %u\n", ret, threshold);
	} else {
		return snprintf(buf, PAGE_SIZE, "Short test PASSED, threshold = %u\n", threshold);
	}
}
...
static DEVICE_ATTR(short_test, S_IWUSR | S_IRUGO, short_test_show, short_test_store);

static ssize_t testing_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t program_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t program_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(program_mode, S_IWUSR | S_IRUGO, program_mode_show, program_mode_store);
...
#define RAWDATA_BUFFER_SIZE(cts_dev) \
	...
	if (rawdata == NULL) {
		return snprintf(buf, PAGE_SIZE, "Allocate memory for rawdata failed\n");
	}

	ret = cts_enable_get_rawdata(cts_dev);
	if (ret) {
		count += snprintf(buf, PAGE_SIZE, "Enable read raw data failed %d\n", ret);
		goto err_free_rawdata;
	}

	ret = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
	if (ret) {
		count += snprintf(buf, PAGE_SIZE, "Send cmd QUIT_GESTURE_MONITOR failed %d\n", ret);
		goto err_free_rawdata;
	}
	msleep(50);
	...
	if (ret) {
		count += snprintf(buf, PAGE_SIZE, "Get raw data failed %d\n", ret);
		data_valid = false;
		/* Fall through to disable get rawdata */
	}
	ret = cts_disable_get_rawdata(cts_dev);
	if (ret) {
		count += snprintf(buf, PAGE_SIZE, "Disable read raw data failed %d\n", ret);
...
#define DIFFDATA_BUFFER_SIZE(cts_dev) \
	...
	diffdata = kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (diffdata == NULL) {
