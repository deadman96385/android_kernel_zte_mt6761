			     ...
			     u8 cmd, void *data, u32 flash_addr, u32 sram_addr, size_t size, u32 flags)
{
	...
	/** - Write data to exchange SRAM if operation is write and
		...
		if ((!(flags & SFCTRLv2_OPFLAG_READ)) && data) {
			...
			if (ret) {
				cts_err("Write data to exchange sram 0x%06x size %zu failed %d", sram_addr, size, ret);
				return ret;
			}
		}
	}
	...
	if ((flags & SFCTRLv2_OPFLAG_READ) && data) {
		...
		if (ret) {
			cts_err("Read data from exchange sram 0x%06x size %zu failed %d", sram_addr, size, ret);
			return ret;
		}
	}
...
static int sfctrlv2_rdid(const struct cts_device *cts_dev, u32 *id)
{
	...
	ret = sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_RDID, buf,
				0, cts_dev->hwdata->sfctrl->xchg_sram_base, 3, SFCTRLv2_CMD_RDID_FLAGS);
...
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_SE, NULL, sector_addr, 0, 0, SFCTRLv2_CMD_SE_FLAGS);
}

static int sfctrlv2_be(const struct cts_device *cts_dev, u32 block_addr)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_BE, NULL, block_addr, 0, 0, SFCTRLv2_CMD_BE_FLAGS);
}

static int sfctrlv2_pp(const struct cts_device *cts_dev, u32 flash_addr, const void *src, size_t size)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_PP, (void *)src,
				 flash_addr, cts_dev->hwdata->sfctrl->xchg_sram_base, size, SFCTRLv2_CMD_PP_FLAGS);
}

static int sfctrlv2_program_flash_from_sram(const struct cts_device *cts_dev, u32 flash_addr, u32 sram_addr, size_t len)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_PP, NULL, flash_addr, sram_addr, len, SFCTRLv2_CMD_PP_FLAGS);
}

static int sfctrlv2_read(const struct cts_device *cts_dev, u32 flash_addr, void *dst, size_t size)
{
...
static int sfctrlv2_read_flash_to_sram(const struct cts_device *cts_dev, u32 flash_addr, u32 sram_addr, size_t size)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_FAST_READ, NULL,
				 flash_addr, sram_addr, size, SFCTRLv2_CMD_FAST_READ_FLAGS);
}

int sfctrlv2_calc_sram_crc(const struct cts_device *cts_dev, u32 sram_addr, size_t size, u32 *crc)
{
	int ret;
...
int sfctrlv2_calc_flash_crc(const struct cts_device *cts_dev, u32 flash_addr, size_t size, u32 *crc)
{
	int ret;
