...
static int erase_sector_retry(const struct cts_device *cts_dev, u32 sector_addr, int retry)
{
	int ret, retries;
	...
	do {
		...
		if (ret) {
			cts_err("Erase sector 0x%06x failed %d retries %d", sector_addr, ret, retries);
			continue;
		}
	} while (retries < retry);
...
/** Make sure sector addr is sector aligned && < flash total size */
static inline int erase_sector(const struct cts_device *cts_dev, u32 sector_addr)
{
...
static int erase_block_retry(const struct cts_device *cts_dev, u32 block_addr, int retry)
{
	int ret, retries;
	...
	do {
		...
		if (ret) {
			cts_err("Erase block 0x%06x failed %d retries %d", block_addr, ret, retries);
			continue;
		}
	} while (retries < retry);
...
int cts_read_flash_retry(const struct cts_device *cts_dev, u32 flash_addr, void *dst, size_t size, int retry)
{
	...
	if (flash == NULL || sfctrl == NULL || sfctrl->ops == NULL || sfctrl->ops->read == NULL) {
		cts_err("Read not supported");
		return -ENOTSUPP;
	}

	if (flash_addr > flash->total_size) {
		cts_err("Read from 0x%06x > flash size 0x%06zx", flash_addr, flash->total_size);
		return -EINVAL;
	}
	...
	while (size) {
		...
		if (ret < 0) {
			cts_err("Read from 0x%06x size %zu failed %d", flash_addr, size, ret);
			return ret;
		}
				 ...
				 u32 flash_addr, u32 sram_addr, size_t size, int retry)
{
	...
	cts_info("Read from 0x%06x to sram 0x%06x size %zu", flash_addr, sram_addr, size);

	sfctrl = cts_dev->hwdata->sfctrl;
	flash = cts_dev->flash;

	if (flash == NULL || sfctrl == NULL || sfctrl->ops == NULL || sfctrl->ops->read_to_sram == NULL) {
		cts_err("Read to sram not supported");
		return -ENOTSUPP;
	}

	if (flash_addr > flash->total_size) {
		cts_err("Read to sram from 0x%06x > flash size 0x%06zx", flash_addr, flash->total_size);
		return -EINVAL;
	}
	size = min(size, flash->total_size - flash_addr);
					   ...
					   u32 flash_addr, u32 sram_addr, size_t size, u32 crc, int retry)
{
	...
	cts_info("Read from 0x%06x to sram 0x%06x size %zu with crc check", flash_addr, sram_addr, size);
	...
	if (flash == NULL ||
	    sfctrl == NULL ||
	    sfctrl->ops == NULL || sfctrl->ops->read_to_sram == NULL || sfctrl->ops->calc_sram_crc == NULL) {
	...
	cts_info("Read to sram actually from 0x%06x size %zu with crc check", flash_addr, size);
	...
	do {
		...
		if (ret) {
			cts_err("Get crc for read from 0x%06x to sram 0x%06x size %zu "
				"failed %d retries %d", flash_addr, sram_addr, size, ret, retries);
			continue;
		}
		...
		cts_err("Check crc for read from 0x%06x to sram 0x%06x size %zu "
			"mismatch %x != %x, retries %d", flash_addr, sram_addr, size, crc, crc_sram, retries);
		ret = -EFAULT;
	} while (retries < retry);
...
int cts_program_flash(const struct cts_device *cts_dev, u32 flash_addr, const void *src, size_t size)
{
	const struct cts_sfctrl *sfctrl;
	const struct cts_flash *flash;
	int ret;
	...
	if (flash == NULL || sfctrl == NULL || sfctrl->ops == NULL || sfctrl->ops->program == NULL) {
		cts_err("Program not supported");
		return -ENOTSUPP;
	}

	if (flash_addr >= flash->total_size) {
		cts_err("Program from 0x%06x >= flash size 0x%06zx", flash_addr, flash->total_size);
		return -EINVAL;
	}
	size = min(size, flash->total_size - flash_addr);
...
int cts_program_flash_from_sram(const struct cts_device *cts_dev, u32 flash_addr, u32 sram_addr, size_t size)
{
	const struct cts_sfctrl *sfctrl;
	const struct cts_flash *flash;
	int ret;

	cts_info("Program to 0x%06x from sram 0x%06x size %zu", flash_addr, sram_addr, size);

	sfctrl = cts_dev->hwdata->sfctrl;
	flash = cts_dev->flash;

	if (flash == NULL || sfctrl == NULL || sfctrl->ops == NULL || sfctrl->ops->program_from_sram == NULL) {
	...
	cts_info("Program actually to 0x%06x from sram 0x%06x size %zu", flash_addr, sram_addr, size);
	...
	while (size) {
		...
		if (ret) {
			cts_err("Program to 0x%06x from sram 0x%06x size %zu failed %d", flash_addr, sram_addr, l, ret);
			return ret;
		}
...
int cts_erase_flash(const struct cts_device *cts_dev, u32 addr, size_t size)
{
	...
	if (flash == NULL ||
	    ...
	    sfctrl->ops->se == NULL || sfctrl->ops->be == NULL || flash == NULL) {
		cts_err("Oops");
		return -EINVAL;
	}
	...
	if (addr > flash->total_size) {
		cts_err("Erase from 0x%06x > flash size 0x%06zx", addr, flash->total_size);
		return -EINVAL;
	}
	...
	if (flash->block_size) {
		while (addr != ALIGN(addr, flash->block_size) && size >= flash->sector_size) {
			ret = erase_sector(cts_dev, addr);
			if (ret) {
				cts_err("Erase sector 0x%06x size 0x%04zx failed %d", addr, flash->sector_size, ret);
				return ret;
			}
		...
		while (size >= flash->block_size) {
			...
			if (ret) {
				cts_err("Erase block 0x%06x size 0x%04zx failed %d", addr, flash->block_size, ret);
				return ret;
			}
