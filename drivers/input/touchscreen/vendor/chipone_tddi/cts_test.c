...
static int get_test_result(struct cts_device *cts_dev, u16 *result)
{
	...
	ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_RAW_DATA, result, TEST_RESULT_BUFFER_SIZE(cts_dev));
	if (ret) {
...
static void dump_test_data(struct cts_device *cts_dev, const char *desc, const u16 *data)
{
...
#define DATA_FORMAT_STR     "%-5u "
	...
	printk(SPLIT_LINE_STR
	       ...
	       SPLIT_LINE_STR "   | ", desc, min_r, min_c, min, max_r, max_c, max, average);
	for (c = 0; c < cts_dev->fwdata.cols; c++) {
...
static int validate_test_result(struct cts_device *cts_dev, const char *desc, u16 *test_result, u16 threshold)
{
#define SPLIT_LINE_STR \
	"------------------------------\n"
...
static int test_short_to_gnd(struct cts_device *cts_dev, u8 feature_ver, u16 *test_result, u16 threshold)
{
	int ret;
...
static int test_short_to_gnd_legacy(struct cts_device *cts_dev, u16 *test_result, u16 threshold)
{
	int ret;
...
static int test_short_between_cols(struct cts_device *cts_dev, u16 *test_result, u16 threshold, bool skip_first_frame)
{
	int ret;
...
static int test_short_between_rows(struct cts_device *cts_dev, u16 *test_result, u16 threshold)
{
	int ret;
	int loopcnt;
...
int cts_short_test(struct cts_device *cts_dev, u16 threshold)
{
	...
	test_result = kmalloc(TEST_RESULT_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (test_result == NULL) {
	...
	ret = test_short_between_cols(cts_dev, test_result, threshold, need_display_on == 0);
	if (ret) {
