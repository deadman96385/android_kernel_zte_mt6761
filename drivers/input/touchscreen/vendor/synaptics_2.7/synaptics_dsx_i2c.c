...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	retval = of_property_read_string(np, "synaptics,pwr-avdd-name", &name);
	if (retval < 0)
		bdata->pwr_avdd_name = NULL;
	else
		bdata->pwr_avdd_name = name;

	retval = of_property_read_string(np, "synaptics,pwr-iovdd-name", &name);
	if (retval < 0)
		bdata->pwr_iovdd_name = NULL;
	else
		bdata->pwr_iovdd_name = name;
	#endif
