		...
		bool enable)
{
	...
	unsigned char reporting_control_val[3] = {0};
	...
	for (i = 0; i < 3; i++) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + offset,
			reporting_control_val, sizeof(reporting_control_val));
#ifdef REPORTTING_CONTROL_8BIT
		pr_notice("%s: reporting_control_val=0x%02x\n", __func__, reporting_control_val[0]);
		if ((enable && (reporting_control_val[0] != F12_WAKEUP_GESTURE_MODE)) ||
			(!enable && (reporting_control_val[0] != F12_CONTINUOUS_MODE))) {
#else
		pr_notice("%s: reporting_control_val=0x%02x\n", __func__, reporting_control_val[2]);
		if ((enable && (reporting_control_val[2] != F12_WAKEUP_GESTURE_MODE)) ||
			(!enable && (reporting_control_val[2] != F12_CONTINUOUS_MODE))) {
#endif
			msleep(20);
			retval = synaptics_rmi4_reg_write(rmi4_data,
				fhandler->full_addr.ctrl_base + offset,
			...
			pr_notice("%s: change reporting mode success\n", __func__);
			break;
		}
	}
...
#endif
	/* print tp point */
	static char finger_down[MAX_POINTS_SUPPORT] = {0};
	int log_x = 0;
	int log_y = 0;
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (gesture_type == 0) {
			...
			retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(rmi4_data->gesture_detection));
			gesture_type = rmi4_data->gesture_detection[0];
			pr_notice("%s: read again gesture_type=0x%02x\n", __func__, gesture_type);
		}
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (stylus_presence)
				break;
			/* print tp point */
			if (!finger_down[finger]) {
				log_x = x;
				log_y = y;
				finger_down[finger] = 1;
				pr_notice(" %s touch down id: %d, coord [%d:%d]\n", __func__, finger, log_x, log_y);
			}
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(rmi4_data->input_dev, finger);
			input_mt_report_slot_state(rmi4_data->input_dev,
		...
		bool get)
{
	...
	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		...
		regulator_set_voltage(rmi4_data->pwr_reg, 3000000, 3000000);
	}
	...
	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		...
		regulator_set_voltage(rmi4_data->bus_reg, 1800000, 1800000);
	}
	...
	int *p_fw_ver)
{
	...
	case 'X':
		...
		break;
	case 'c':
		snprintf(p_chip_type, CHIP_INFO_LEN, "S3608(0x%x)", ts->config_id.chip_type);
		break;
	default:
	...
	case 'W':
		...
		break;
	case 'X':
		snprintf(p_sensor, CHIP_INFO_LEN, "ZGSAIHUA(0x%x)", ts->config_id.sensor);
		touch_module = ZGSAIHUA;
		break;
	default:
