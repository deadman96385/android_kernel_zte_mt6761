...
#define AMP_OPEN_TEST_LIMIT_PHASE1_UPPER 4000 /* ( unit = femtofarad (fF)) */
#define AMP_OPEN_TEST_LIMIT_PHASE2_LOWER 70   /* ( unit = ratio) */
#define AMP_OPEN_TEST_LIMIT_PHASE2_UPPER 160  /* ( unit = ratio) */
		...
		struct device_attribute *attr, char *buf)
{
	...
	} else if ((f21 != NULL) && f21->has_force) {
...
static short find_median(short *pdata, int num)
{
	...
	value = kcalloc(num, sizeof(short), GFP_KERNEL);
...
static int tddi_ratio_calculation(signed short *p_image)
{
	...
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	int left_size = (int)f54->left_mux_size;
	int right_size = (int)f54->right_mux_size;
	...
	p_right_median = kcalloc(rx_num, sizeof(short), GFP_KERNEL);
	if (!p_right_median) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	p_left_median = kcalloc(rx_num, sizeof(short), GFP_KERNEL);
	if (!p_left_median) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	p_right_column_buf = kcalloc(right_size * rx_num, sizeof(short), GFP_KERNEL);
	...
	p_left_column_buf = kzalloc(left_size * rx_num * sizeof(short), GFP_KERNEL);
...
int tpd_synaptics_get_full_rawdata(int *rawdata, int length)
{
	...
	int count = 0;
	unsigned char *pchar_raw_data_buffer = NULL;

	pr_notice("%s:\n", __func__);

	if (rawdata == NULL || length < tx_num * rx_num)
		return -EINVAL;

	retval = test_sysfs_read_report(NULL, NULL, "3", 2, false, false);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read report. exit\n",
				__func__);
		return retval;
	}

	pchar_raw_data_buffer = kzalloc(full_raw_report_size, GFP_KERNEL);
	if (!pchar_raw_data_buffer) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = secure_memcpy(pchar_raw_data_buffer, full_raw_report_size,
		f54->report_data, f54->report_size, full_raw_report_size);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to copy report data\n", __func__);
		return retval;
	}
	report_data_16 = (short *)pchar_raw_data_buffer;
	count = 0;

	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
			rawdata[count] = (signed)*report_data_16;
			pr_notice("#%-4d %-4d ", *report_data_16,  rawdata[count]);
			report_data_16++;
			count++;
		}
		pr_notice("\n");
	}
	pr_notice("print td43xx full raw data\n");
	report_data_16 = (short *)pchar_raw_data_buffer;
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			pr_notice("#%-4d %-4d ", *report_data_16,  rawdata[ii * rx_num + jj]);
			report_data_16++;

		}
		pr_notice("#%-4d %-4d\n", *report_data_16,  rawdata[ii * rx_num + rx_num - 1]);
		report_data_16++;
	}

	kfree(pchar_raw_data_buffer);
...
int tpd_synaptics_amp_open_test(struct synaptics_test_st *stp_test)
{
	...
	unsigned char *pchar_amp_open_data_buffer = NULL;

	pchar_amp_open_data_buffer = kzalloc(tx_num * rx_num, GFP_KERNEL);
	if (!pchar_amp_open_data_buffer) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = test_sysfs_read_report(NULL, NULL, "3", 2,
				false, false);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = secure_memcpy(p_report_data_8, tx_num * rx_num * 2,
		f54->report_data, f54->report_size, tx_num * rx_num * 2);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to copy report data\n", __func__);
		goto exit;
	...
	save_ito_array_data_to_file(stp_test, "amp open data 1", p_rt92_image_1,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	control.reg_99->reset_duration = original_data_f54_ctrl99[2];
	pr_notice("synaptics : amp open test setp2 dur lsb:0x%x msb:0x%x rst:0x%x\n",
		control.reg_99->integration_duration_lsb,
		control.reg_99->integration_duration_msb,
		control.reg_99->reset_duration);
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_99->address,
			control.reg_99->data,
			sizeof(control.reg_99->data));
	...
	save_ito_array_data_to_file(stp_test, "amp open data 2", p_rt92_image_2,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "amp open delta data", p_rt92_delta_image,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if ((p_rt92_delta_image[i * rx_num + j] <
				stp_test->basic_threshold.amp_open_test_limit_phase1_lower) ||
				(p_rt92_delta_image[i * rx_num + j] >
				stp_test->basic_threshold.amp_open_test_limit_phase1_upper)) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d at phase 1 stone (limit = %d, %d)\n",
						__func__, i, j, p_rt92_delta_image[i*rx_num + j],
						AMP_OPEN_TEST_LIMIT_PHASE1_LOWER, AMP_OPEN_TEST_LIMIT_PHASE1_UPPER);

				dev_err(rmi4_data->pdev->dev.parent,
				"%s: lower = %d , upper = %d\n",
				__func__, stp_test->basic_threshold.amp_open_test_limit_phase1_lower,
				stp_test->basic_threshold.amp_open_test_limit_phase1_upper);

				p_rt92_image_1[i*rx_num + j] = _TEST_FAIL; /* 1: fail */

				stp_test->test_result |= TEST_BEYOND_ACCORD_LIMIT | TEST_GT_OPEN;
			} else
				p_rt92_image_1[i*rx_num + j] = _TEST_PASS;
		}
	}

	save_ito_array_data_to_file(stp_test, "amp open test phase1 limit: ", p_rt92_image_1,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "amp open test ratio data: ", p_rt92_delta_image,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if ((p_rt92_delta_image[i * rx_num + j] <
			stp_test->basic_threshold.amp_open_test_limit_phase2_lower) ||
				(p_rt92_delta_image[i * rx_num + j] >
				stp_test->basic_threshold.amp_open_test_limit_phase2_upper)) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
				...
				dev_err(rmi4_data->pdev->dev.parent,
				"%s: phase 2   lower = %d , upper = %d\n",
				__func__, stp_test->basic_threshold.amp_open_test_limit_phase2_lower,
				stp_test->basic_threshold.amp_open_test_limit_phase2_upper);
	...
	save_ito_array_data_to_file(stp_test, "amp open test phase2 limit: ", p_rt92_image_2,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	for (i = 0; i < tx_num; i++)
		for (j = 0; j < rx_num; j++)
			pchar_amp_open_data_buffer[i * rx_num + j] =
...
exit:
	...
	kfree(pchar_amp_open_data_buffer);

	test_sysfs_resume_touch_store(NULL, NULL, "1", 1);
...
int tpd_synaptics_ee_short_test(struct synaptics_test_st *stp_test)
{
	...
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	unsigned char *pchar_ee_short_data_buffer;


	/* allocate the g_tddi_ee_short_data_output */
	pchar_ee_short_data_buffer = kzalloc(tx_num * rx_num, GFP_KERNEL);
	if (!pchar_ee_short_data_buffer) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	/* step 1 */
	/* get report image 95 */
	retval = test_sysfs_read_report(NULL, NULL, "95", 2,
				false, false);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 1 image", tddi_rt95_part_one,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (tddi_rt95_part_one[i*rx_num + j] > stp_test->basic_threshold.ee_short_test_limit_part1) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d in part 1 image (limit = %d)\n",
						__func__, i, j, tddi_rt95_part_one[i * rx_num + j],
						EE_SHORT_TEST_LIMIT_PART1);

				tddi_rt95_part_one[i*rx_num + j] = _TEST_FAIL; /* 1: fail */

				stp_test->test_result |= TEST_GT_SHORT;
			} else
				tddi_rt95_part_one[i*rx_num + j] = _TEST_PASS;
		}
	}
	save_ito_array_data_to_file(stp_test, "eeshort test part 1 limit", tddi_rt95_part_one,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 2 image", tddi_rt95_part_two,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);

	/* calculate the ratio */
	tddi_ratio_calculation(tddi_rt95_part_two);
	save_ito_array_data_to_file(stp_test, "eeshort test calculate the ratio", tddi_rt95_part_two,
		tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (tddi_rt95_part_two[i*rx_num + j] <
				stp_test->basic_threshold.ee_short_test_limit_part2) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d in part 2 image (limit = %d)\n",
						__func__, i, j, tddi_rt95_part_two[i*rx_num + j],
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 2 limit",
		tddi_rt95_part_two, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	for (i = 0; i < tx_num; i++)
		for (j = 0; j < rx_num; j++)
			pchar_ee_short_data_buffer[i * rx_num + j] =
				(unsigned char)(tddi_rt95_part_one[i * rx_num + j]) ||
				tddi_rt95_part_two[i * rx_num + j];
...
exit:
	...
	kfree(pchar_ee_short_data_buffer);
