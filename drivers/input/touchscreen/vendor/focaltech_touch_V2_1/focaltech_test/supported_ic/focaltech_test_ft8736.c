...
bool start_test_ft8736(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_ENTER_FACTORY_MODE) {
			ret = ft8736_enter_factory_mode();
			if (ret != ERROR_CODE_OK || (!tmp_result)) {
				test_result = false;
		...
		/* FT8736_RAWDATA_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_RAWDATA_TEST) {
		...
		/* FT8736_CB_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_CB_TEST) {
		...
		/* FT8736_SHORT_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_SHORT_CIRCUIT_TEST) {
		...
		/* FT8736_Open_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_OPEN_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8736_LCD_NOISE_TEST) {
...
static int ft8736_rawdata_test(bool *test_result)
{
	...
	tmp_result =
		compare_detailthreshold_data_incell(rawdata, test_data.incell_detail_thr.rawdata_test_min,
				test_data.incell_detail_thr.rawdata_test_max, include_key);
...
static int ft8736_cb_test(bool *test_result)
{
	...
	for (col = 0; col < read_key_len /*test_data.screen_param.key_num_total */ ; ++col) {
		if (uc_bits != 0) {
			cbdata[test_data.screen_param.tx_num * test_data.screen_param.rx_num + col / 2] =
				((cbdata[test_data.screen_param.tx_num * test_data.screen_param.rx_num + col] &
				  0x01) << 8) +
				cbdata[test_data.screen_param.tx_num * test_data.screen_param.rx_num + col + 1];
			col++;
		} else {
			cbdata[test_data.screen_param.tx_num * test_data.screen_param.rx_num + col] =
				cbdata[test_data.screen_param.tx_num * test_data.screen_param.rx_num + col];
...
static int ft8736_short_test(bool *test_result)
{
	...
	/*****************************************************
	Before ShortCircuitTest need LCD driver to control power off
	******************************************************/
...
static int ft8736_lcdnoise_test(bool *test_result)
{
	...
	bytes_num =
		2 * (test_data.screen_param.tx_num * test_data.screen_param.rx_num +
		     test_data.screen_param.key_num_total);
	...
	if (ft8736_basic_thr.lcd_noise_test_noise_mode == 0) {
		for (row = 0; row <= test_data.screen_param.tx_num + 1; ++row) {
			for (col = 0; col < test_data.screen_param.rx_num; ++col) {
	...
	if (ft8736_basic_thr.lcd_noise_test_noise_mode == 1) {
		for (row = 0; row <= test_data.screen_param.tx_num + 1; ++row) {
			for (col = 0; col < test_data.screen_param.rx_num; ++col) {
				lcdnoise[row * test_data.screen_param.rx_num + col] =
					sqrt_new(lcdnoise[row * test_data.screen_param.rx_num + col] / reg_data);
			}
		}
	}
