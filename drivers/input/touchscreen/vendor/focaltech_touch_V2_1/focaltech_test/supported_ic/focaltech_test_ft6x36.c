...
static void Save_Test_Data(int iData[MAX_SCAP_CHANNEL_NUM], int iArrayIndex, unsigned char Row,
...
bool start_test_ft6x36(void)
{
	...
	for (iItemCount = 0; iItemCount < test_data.test_num; iItemCount++) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT6X36_ENTER_FACTORY_MODE) {
		...
		/* FT6X36_RAWDATA_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT6X36_RAWDATA_TEST) {
		...
		/* FT6X36_CB_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT6X36_CB_TEST) {
		...
		/* Code_FT6X36_DELTA_CB_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT6X36_DELTA_CB_TEST) {
...
unsigned char FT6X36_TestItem_RawDataTest(bool *bTestResult)
{
	...
	for (i = 0; i < test_data.screen_param.channels_num + test_data.screen_param.key_num; i++) {
		...
		if (m_RawData[i] < RawDataMin || m_RawData[i] > RawDataMax) {
			btmpresult = false;
			fts_save_failed_node(0, test_data.screen_param.channels_num, 0, i);
			if (iNgNum == 0)
				FTS_TEST_SAVE_INFO("\n \r\n======= NG Data: \r");

			if (i < test_data.screen_param.channels_num)
				FTS_TEST_SAVE_INFO("\n Ch_%02d: %d Set_Range=(%d, %d) ,	", i + 1,
						   m_RawData[i], RawDataMin, RawDataMax);
			else
				FTS_TEST_SAVE_INFO("\n Key_%d: %d Set_Range=(%d, %d) ,	",
						   i + 1 - test_data.screen_param.channels_num, m_RawData[i],
	...
	FTS_TEST_SAVE_INFO
	("\n \r\n\r\n Max Raw Value: %d, Min Raw Value: %d, Deviation Value: %d, Average Value: %d", iMax,
	 iMin, iMax - iMin, iAvg);

	/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Collect test data and save CSV file. */

	Save_Test_Data(m_RawData, 0, 1, test_data.screen_param.channels_num + test_data.screen_param.key_num,
		       1);
...
unsigned char FT6X36_TestItem_CbTest(bool *bTestResult)
{
	...
	if ((ReCode != ERROR_CODE_OK) /* || (1 != WaterProofResult) */) {
		...
		for (i = 0; i < test_data.screen_param.channels_num + test_data.screen_param.key_num; i++) {
			...
			if (m_TempCbData[i] < iCbMin || m_TempCbData[i] > iCbMax) {
				...
				if (i < test_data.screen_param.channels_num)
					FTS_TEST_SAVE_INFO("\n Ch_%02d: %d Set_Range=(%d, %d) ,	", i + 1,
							   m_TempCbData[i], iCbMin, iCbMax);
				else
					FTS_TEST_SAVE_INFO("\n Key_%d: %d Set_Range=(%d, %d),	",
							   i + 1 - test_data.screen_param.channels_num,
							   m_TempCbData[i], iCbMin, iCbMax);
				if (iNgNum % 6 == 0) {
					FTS_TEST_SAVE_INFO("\n \r");
				}
		...
		iAvg /= test_data.screen_param.channels_num + test_data.screen_param.key_num;
		FTS_TEST_SAVE_INFO
		("\n \r\n\r\n Max CB Value: %d, Min CB Value: %d, Deviation Value: %d, Average Value: %d",
	...
	Save_Test_Data(m_TempCbData, 0, 1,
		       test_data.screen_param.channels_num + test_data.screen_param.key_num, 1);
	return ReCode;
...
unsigned char FT6X36_TestItem_DeltaCbTest(bool *bTestResult)
{
	...
	if (bUseSort1) {
		if (Delta_Ci_Differ <= Sort1Max - Sort1Min) {
			if (bUseCriticalValue) {
				...
				FTS_TEST_SAVE_INFO
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort1Max, Sort1Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical Deviation of Sort1: %d",
				 Sort1Max - Sort1Min, Critical_Delta_S1);
			} else {
				btmpresult = false;
				FTS_TEST_SAVE_INFO
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort1Max, Sort1Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort1: %d",
				 Sort1Max - Sort1Min, Delta_Ci_Differ);
			}
		} else {
			FTS_TEST_SAVE_INFO
			("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
		...
		if (Delta_Ci_Differ_S2 <= Sort2Max - Sort2Min) {
			if (bUseCriticalValue) {
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort2Max, Sort2Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical Deviation of Sort2: %d",
				 Sort2Max - Sort2Min, Critical_Delta_S2);
			} else {
				btmpresult = false;
				FTS_TEST_SAVE_INFO
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort2Max, Sort2Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort2: %d",
				 Sort2Max - Sort2Min, Delta_Ci_Differ_S2);
			}
		} else {
			FTS_TEST_SAVE_INFO
			("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
		...
		if (Delta_Ci_Differ_S3 <= Sort3Max - Sort3Min) {
			if (bUseCriticalValue) {
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort3Max, Sort3Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical Deviation  of Sort3: %d",
				 Sort3Max - Sort3Min, Critical_Delta_S3);
			} else {
				btmpresult = false;
				FTS_TEST_SAVE_INFO
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort3Max, Sort3Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort3: %d",
				 Sort3Max - Sort3Min, Delta_Ci_Differ_S3);
			}
		} else {
			FTS_TEST_SAVE_INFO
			("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
		...
		if (Delta_Ci_Differ_S4 <= Sort4Max - Sort4Min) {
			if (bUseCriticalValue) {
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort4Max, Sort4Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical  Deviation of Sort4: %d",
				 Sort4Max - Sort4Min, Critical_Delta_S4);
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort4Max, Sort4Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort4: %d",
				 Sort4Max - Sort4Min, Delta_Ci_Differ_S4);
		...
		if (Delta_Ci_Differ_S5 <= Sort5Max - Sort5Min) {
			if (bUseCriticalValue) {
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort5Max, Sort5Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical  Deviation of Sort5: %d",
				 Sort5Max - Sort5Min, Critical_Delta_S5);
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort5Max, Sort5Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort5: %d",
				 Sort5Max - Sort5Min, Delta_Ci_Differ_S5);
			}
		} else {
			FTS_TEST_SAVE_INFO
			("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
		...
		if (Delta_Ci_Differ_S6 <= Sort6Max - Sort6Min) {
			if (bUseCriticalValue) {
				...
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort6Max, Sort6Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Critical  Deviation of Sort6: %d",
				 Sort6Max - Sort6Min, Critical_Delta_S6);
			} else {
				btmpresult = false;
				FTS_TEST_SAVE_INFO
				("\n \r\n\r\n Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d,",
				 Sort6Max, Sort6Min);
				FTS_TEST_SAVE_INFO
				("Get Deviation: %d, Set Deviation of Sort6: %d",
				 Sort6Max - Sort6Min, Delta_Ci_Differ_S6);
	...
	FTS_TEST_SAVE_INFO
	("\n \r\n\r\n Condition2: Get Max Differ Data of Delta_CB(abs): %d, Set Max Differ Data of Delta_CB(abs): %d",
	...
	if (g_stCfg_FT6X36_BasicThreshold.DeltaCbTest_Include_Key_Test) {
		...
		for (i = test_data.screen_param.channels_num;
		     i < test_data.screen_param.channels_num + test_data.screen_param.key_num; i++) {
		...
		FTS_TEST_SAVE_INFO
		("\n \r\n\r\n Condition3: Include Key Test, Get Max Key Data: %d, Set Max Key Data: %d",
		 Key_Delta_Max, SetKeyMax);
	}
	...
	Save_Test_Data(m_DeltaCbData, 0, 1,
		       test_data.screen_param.channels_num + test_data.screen_param.key_num, 1);

	/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Collect test data and save CSV file. */

	Save_Test_Data(m_DeltaCb_DifferData, 0, 1,
		       test_data.screen_param.channels_num + test_data.screen_param.key_num, 2);

	/* Collection of Ci Data Delta, into the CSV file */
	/* GetDeltaCiDataMsg(); */
	return 0;
}
...
* Input: int iData[TX_NUM_MAX][RX_NUM_MAX], int iArrayIndex, unsigned char Row,
	unsigned char Col, unsigned char ItemCount
* Output: none
* Return: none
***********************************************************************/
			   ...
			   unsigned char Col, unsigned char ItemCount)
{
	...
	iLen = snprintf(test_data.tmp_buffer, PAGE_SIZE, "NA, %d, %d, %d, %d, %d, ",
	...
	for (i = 0 + iArrayIndex; i < Row + iArrayIndex; i++) {
		for (j = 0; j < Col; j++) {
			if (j == (Col - 1))	/* The Last Data of the Row, add "\n" */
				...
				iLen = snprintf(test_data.tmp_buffer, PAGE_SIZE, "%d, ", iData[j]);

			memcpy(test_data.store_data_area + test_data.len_store_data_area,
			       test_data.tmp_buffer, iLen);
...
void init_basicthreshold_ft6x36(char *strIniFile)
{
	...
	snprintf(g_stCfg_FT6X36_BasicThreshold.Project_Code,
		 sizeof(g_stCfg_FT6X36_BasicThreshold.Project_Code), "%s", str);
	...
	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S1", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S1 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S2", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S2 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S3", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S3 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S4", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S4 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S5", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S5 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Deviation_S6", "8", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Deviation_S6 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Set_Critical", "0", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Set_Critical = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S1", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S1 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S2", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S2 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S3", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S3 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S4", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S4 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S5", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S5 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelsDeviationTest_Critical_S6", "13", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Critical_S6 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S1", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S1 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S2", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S2 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S3", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S3 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S4", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S4 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S5", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S5 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Deviation_S6", "5", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Deviation_S6 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Set_Critical", "0", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Set_Critical = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S1", "10", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Critical_S1 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S2", "10", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Critical_S2 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S3", "10", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Critical_S3 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S4", "10", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Critical_S4 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S5", "10", str,
				strIniFile);
	g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Critical_S5 = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "TwoSidesDeviationTest_Critical_S6", "10", str,
				strIniFile);
...
void init_detailthreshold_ft6x36(char *ini)
{
	...
	memcpy(test_data.scap_detail_thr.rawdata_test_max, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("RawDataTest_Min", ini, 16500);
	memcpy(test_data.scap_detail_thr.rawdata_test_min, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("CiTest_Max", ini, 5);
	memcpy(test_data.scap_detail_thr.CiTest_Max, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("CiTest_Min", ini, 250);
	memcpy(test_data.scap_detail_thr.CiTest_Min, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCiTest_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.DeltaCiTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCiTest_AnotherBase1", ini, 0);
	memcpy(test_data.scap_detail_thr.DeltaCiTest_AnotherBase1, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCiTest_AnotherBase2", ini, 0);
	memcpy(test_data.scap_detail_thr.DeltaCiTest_AnotherBase2, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("NoiseTest_Max", ini, 20);
	memcpy(test_data.scap_detail_thr.NoiseTest_Max, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("CiDeviation_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.CiDeviationTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCxTest_Sort", ini, 1);
	memcpy(test_data.scap_detail_thr.DeltaCxTest_Sort, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCxTest_Area", ini, 1);
	memcpy(test_data.scap_detail_thr.DeltaCxTest_Area, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	/* 6x36 */
	OnGetTestItemParam("CbTest_Max", ini, 0);
	memcpy(test_data.scap_detail_thr.cb_test_max, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("CbTest_Min", ini, 0);
	memcpy(test_data.scap_detail_thr.cb_test_min, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DeltaCbTest_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.DeltaCbTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("DifferTest_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.DifferTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("CBDeviation_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.CBDeviationTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));

	OnGetTestItemParam("K1DifferTest_Base", ini, 0);
	memcpy(test_data.scap_detail_thr.K1DifferTest_Base, test_data.scap_detail_thr.TempData,
	       MAX_CHANNEL_NUM * sizeof(int));
