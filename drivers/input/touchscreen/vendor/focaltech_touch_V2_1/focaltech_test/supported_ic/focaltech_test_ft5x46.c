...
static void Save_Test_Data(int iData[TX_NUM_MAX][RX_NUM_MAX], char *test_num, int iArrayIndex,
			   unsigned char Row, unsigned char Col, unsigned char ItemCount);
...
bool start_test_ft5x46(void)
{
	...
	for (iItemCount = 0; iItemCount < test_data.test_num; iItemCount++) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_ENTER_FACTORY_MODE) {
		...
		/* FT5X46_RAWDATA_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_RAWDATA_TEST) {
		...
		/* FT5X46_SCAP_CB_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_SCAP_CB_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_SCAP_RAWDATA_TEST) {
		...
		/* Code_FT5X46_PANELDIFFER_TEST, */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_PANELDIFFER_TEST) {
		...
		/* Code_FT5X46_WEAK_SHORT_CIRCUIT_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5X46_WEAK_SHORT_CIRCUIT_TEST) {
...
unsigned char FT5X46_TestItem_RawDataTest(bool *bTestResult)
{
	...
	if (test_data.screen_param.normalize == AUTO_NORMALIZE) {
		...
		/* Save Test Data */
		Save_Test_Data(m_RawData, "RawData Test", 0, test_data.screen_param.tx_num,
			...
			for (iRow = 0; iRow < test_data.screen_param.tx_num; iRow++) {
				...
				for (iCol = 0; iCol < test_data.screen_param.rx_num; iCol++) {
					if (test_data.mcap_detail_thr.invalid_node[iRow][iCol] == 0)
						continue;	/* Invalid Node */
					RawDataMin =
						test_data.mcap_detail_thr.rawdata_test_low_min[iRow][iCol];
					RawDataMax =
						test_data.mcap_detail_thr.rawdata_test_low_max[iRow][iCol];
					iValue = m_RawData[iRow][iCol];
					if (iValue < RawDataMin || iValue > RawDataMax) {
						btmpresult = false;
						fts_save_failed_node(test_data.screen_param.tx_num,
...
unsigned char FT5X46_TestItem_SCapRawDataTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		memcpy(m_RawData[1 + test_data.screen_param.tx_num],
		       m_iTempRawData + test_data.screen_param.rx_num,
	...
	if (g_stCfg_FT5X46_BasicThreshold.SCapRawDataTest_SetWaterproof_ON && bFlag) {
		...
		if (iCount == 0) {
			...
			iAvg = iAvg / iCount;
		Save_Test_Data(m_RawData, "SCap RawData Test", test_data.screen_param.tx_num + 0, 2,
			       test_data.screen_param.rx_num, 1);
	}
	/* Waterproof OFF */
	...
	if (g_stCfg_FT5X46_BasicThreshold.SCapRawDataTest_SetWaterproof_OFF && bFlag) {
		...
		if (iCount == 0) {
			...
			iAvg = iAvg / iCount;
		Save_Test_Data(m_RawData, "SCap RawData Test", test_data.screen_param.tx_num + 2, 2,
			       test_data.screen_param.rx_num, 2);
	}
	/* -----4. post-stage work */
	if (m_bV3TP) {
...
unsigned char FT5X46_TestItem_SCapCbTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ReCode =
			GetTxSC_CB(test_data.screen_param.tx_num + test_data.screen_param.rx_num + 128,
				   m_ucTempData);
		if (ReCode != ERROR_CODE_OK) {
			FTS_TEST_SAVE_INFO("\nGetTxSC_CB Failed!");
			goto TEST_ERR;
		...
		for (index = 0; index < test_data.screen_param.tx_num; ++index) {
			m_RawData[1 + test_data.screen_param.tx_num][index] =
				m_ucTempData[index + test_data.screen_param.rx_num];
		}
		...
		for (index = 0; index < test_data.screen_param.tx_num; ++index) {
			m_RawData[3 + test_data.screen_param.tx_num][index] =
				m_ucTempData[index + test_data.screen_param.rx_num];
		}
	...
	if (g_stCfg_FT5X46_BasicThreshold.SCapCbTest_SetWaterproof_ON && bFlag) {
		...
		if (iCount == 0) {
			...
			iAvg = iAvg / iCount;
		Save_Test_Data(m_RawData, "SCap CB Test", test_data.screen_param.tx_num + 0, 2,
			       test_data.screen_param.rx_num, 1);
	}
	...
	if (g_stCfg_FT5X46_BasicThreshold.SCapCbTest_SetWaterproof_OFF && bFlag) {
		...
		if (iCount == 0) {
			...
			iAvg = iAvg / iCount;
		Save_Test_Data(m_RawData, "SCap CB Test", test_data.screen_param.tx_num + 2, 2,
			       test_data.screen_param.rx_num, 2);
	}
	/* -----4. post-stage work */
	if (m_bV3TP) {
...
unsigned char FT5X46_TestItem_PanelDifferTest(bool *bTestResult)
{
	...
	for (i = 0; i < test_data.screen_param.tx_num; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			...
			if (test_data.mcap_detail_thr.invalid_node[i][j] == NODE_AST_TYPE
			    || test_data.mcap_detail_thr.invalid_node[i][j] == NODE_INVALID_TYPE) {
				InvalidNum++;
				continue;
			}
	...
	Save_Test_Data(m_absDifferData, "Panel Differ Test", 0, test_data.screen_param.tx_num,
		       test_data.screen_param.rx_num, 1);
	/* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<get test data ,and save to .csv file */
...
unsigned char FT5X46_TestItem_WeakShortTest(bool *bTestResult)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (ReCode != ERROR_CODE_OK) {
			...
			if (iAdcData[0] == 0 || iAdcData[1] == 0 || iAdcData[0] == 0xFFFF
			    || iAdcData[1] == 0xFFFF) {
				continue;
			}
	...
	for (i = 0; i < iChannelNum; i++) {
		...
		if (IcValue <= 0x05 || IcValue == 0xff) {
			...
			fGShortResistance[i] =
				(iDsen - iDoffset + 410) * 25 * fKcal / (2047 + iDoffset - iDsen) - 3;
		} else {
	...
	for (i = 0; i < iChannelNum; i++) {
		...
		if (IcValue <= 0x05 || IcValue == 0xff) {
			...
			fMShortResistance[i] =
				(iDsen - iDoffset - 123) * iRsen * fKcal / (iDrefn - iDsen /*temp */) - 2;
		}
...
* Input: int iData[TX_NUM_MAX][RX_NUM_MAX], int iArrayIndex,
*	unsigned char Row, unsigned char Col, unsigned char ItemCount
* Output: none
* Return: none
***********************************************************************/
			   ...
			   unsigned char Row, unsigned char Col, unsigned char ItemCount)
{
	...
	iLen = snprintf(test_data.tmp_buffer, PAGE_SIZE, "%s, %d, %d, %d, %d, %d, ",
...
static unsigned char GetRawData(void)
{
	...
	if ((test_data.screen_param.tx_num + test_data.screen_param.rx_num) == 0) {
		ReCode = GetChannelNum();
		if (ReCode != ERROR_CODE_OK) {
			FTS_TEST_SAVE_INFO("\nError Channel Num...");
			return ERROR_CODE_INVALID_PARAM;
...
void init_basicthreshold_ft5x46(char *strIniFile)
{
	...
	snprintf(g_stCfg_FT5X46_BasicThreshold.Project_Code,
		 sizeof(g_stCfg_FT5X46_BasicThreshold.Project_Code), "%s", str);
...
int get_tp_rawdata_ft5x46(char *buffer, int length)
{
	unsigned char ReCode = 0;
	unsigned char ucFre;
	unsigned char ucFir;
	...
	ReCode = enter_factory_mode();
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("Failed to Enter factory Mode. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	...
	if (m_bV3TP) {
		ReCode = read_reg(REG_MAPPING_SWITCH, &strSwitch);
		if (ReCode != ERROR_CODE_OK) {
			FTS_INFO("Read REG_MAPPING_SWITCH error. Error Code: %d", ReCode);
			goto TEST_ERR;
		}
		...
		if (strSwitch != 0) {
			ReCode = write_reg(REG_MAPPING_SWITCH, 0);
			if (ReCode != ERROR_CODE_OK) {
				FTS_INFO("Write REG_MAPPING_SWITCH error. Error Code: %d", ReCode);
				goto TEST_ERR;
			}

			ReCode = GetChannelNum();
			if (ReCode != ERROR_CODE_OK) {
				FTS_INFO("GetChannelNum error. Error Code: %d", ReCode);
				goto TEST_ERR;
			}
		}
	}
	...
	ReCode = read_reg(REG_NORMALIZE_TYPE, &OriginValue);	/* read the original value */
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("Read  REG_NORMALIZE_TYPE error. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	ReCode = read_reg(0x0A, &ucFre);
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("Read  0x0A error. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	ReCode = read_reg(0xFB, &ucFir);
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("Read  0xFB error. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	...
	if (OriginValue != 1) {	/* if original value is not the value needed,write the register to change */
		ReCode = write_reg(REG_NORMALIZE_TYPE, 0x01);
		if (ReCode != ERROR_CODE_OK) {
			FTS_INFO("write  REG_NORMALIZE_TYPE error. Error Code: %d", ReCode);
			goto TEST_ERR;
		}
	}
	/* Set Frequecy High */

	FTS_INFO("=========Set Frequecy High\n");
	ReCode = write_reg(0x0A, 0x81);
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("Set Frequecy High error. Error Code: %d", ReCode);
		goto TEST_ERR;
	}

	FTS_INFO("=========FIR State: ON\n");
	ReCode = write_reg(0xFB, 1);	/* FIR OFF  0:close, 1:open */
	if (ReCode != ERROR_CODE_OK) {
		FTS_INFO("\n\n FIR State: ON error. Error Code: %d", ReCode);
		goto TEST_ERR;
	...
	for (iRow = 0; iRow < test_data.screen_param.tx_num; iRow++) {
		for (iCol = 0; iCol < test_data.screen_param.rx_num; iCol++) {
			...
			if (iLen >= length - (test_data.screen_param.rx_num + 1) * 6)
				break;

			if (iCol == (test_data.screen_param.rx_num - 1))
				iLen += snprintf(buffer + iLen, length, "%d,\n", iValue);
			else
				iLen += snprintf(buffer + iLen, length, "%d,", iValue);
		}
	}
	return iLen;

TEST_ERR:
	iLen += snprintf(buffer, length, "%s\n", "rawdata test failed");
	return iLen;
}
struct test_funcs test_func_ft5x46 = {
	...
	.get_tp_rawdata = get_tp_rawdata_ft5x46,
};
#endif
