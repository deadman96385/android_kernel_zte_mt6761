/************************************************************************
* Copyright (C) 2012-2018, Focaltech Systems (R)¡ê?All Rights Reserved.
*
...
bool start_test_ft5822(void)
{
	...
	for (iItemCount = 0; iItemCount < test_data.test_num; iItemCount++) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT5822_ENTER_FACTORY_MODE) {
		...
		/* FT5822_RAWDATA_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5822_RAWDATA_TEST) {
		...
		/* FT5822_SCAP_CB_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT5822_SCAP_CB_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT5822_SCAP_RAWDATA_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT5822_PANELDIFFER_TEST) {
...
unsigned char FT5822_TestItem_SCapRawDataTest(bool *bTestResult)
{
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapRawDataTest_SetWaterproof_ON && bFlag) {
		...
		for (i = 0; bFlag && i < test_data.screen_param.rx_num; i++) {
			...
			if (Value > RawDataMax || Value < RawDataMin) {
				btmpresult = false;
				FTS_TEST_SAVE_INFO("\nFailed. Num = %d, Value = %d, range = (%d, %d):", i + 1,
		...
		FTS_TEST_SAVE_INFO
		("\nSCap RawData in Waterproof-ON, Max : %d, Min: %d, Deviation: %d, Average: %d", iMax,
		 iMin, iMax - iMin, iAvg);
		/* Save Test Data */
		Save_Test_Data(m_RawData, test_data.screen_param.tx_num + 0, 2, test_data.screen_param.rx_num,
			       1);
	}
	/* Waterproof OFF */
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapRawDataTest_SetWaterproof_OFF && bFlag) {
		...
		FTS_TEST_SAVE_INFO
		("\nSCap RawData in Waterproof-OFF, Max : %d, Min: %d, Deviation: %d, Average: %d", iMax,
		 iMin, iMax - iMin, iAvg);
		/* Save Test Data */
		Save_Test_Data(m_RawData, test_data.screen_param.tx_num + 2, 2, test_data.screen_param.rx_num,
			       2);
	}
	/* -----4. post-stage work */
	if (m_bV3TP) {
...
unsigned char FT5822_TestItem_SCapCbTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ReCode = write_reg(REG_ScCbAddrR, 0);
		ReCode =
			GetTxSC_CB(test_data.screen_param.rx_num + test_data.screen_param.tx_num + 128,
				   m_ucTempData);
		for (index = 0; index < test_data.screen_param.rx_num; ++index) {
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_ON && bFlag) {
		...
		FTS_TEST_SAVE_INFO
		("\nSCap CB in Waterproof-ON, Max : %d, Min: %d, Deviation: %d, Average: %d", iMax, iMin,
		 iMax - iMin, iAvg);
		/* Save Test Data */
		Save_Test_Data(m_RawData, test_data.screen_param.tx_num + 0, 2, test_data.screen_param.rx_num,
			       1);
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_OFF && bFlag) {
		...
		FTS_TEST_SAVE_INFO
		("\nSCap CB in Waterproof-OFF, Max : %d, Min: %d, Deviation: %d, Average: %d", iMax, iMin,
		 iMax - iMin, iAvg);
		/* Save Test Data */
		Save_Test_Data(m_RawData, test_data.screen_param.tx_num + 2, 2, test_data.screen_param.rx_num,
			       2);
...
void init_basicthreshold_ft5822(char *strIniFile)
{
	...
	snprintf(g_stCfg_FT5822_BasicThreshold.Project_Code,
		 sizeof(g_stCfg_FT5822_BasicThreshold.Project_Code), "%s", str);
