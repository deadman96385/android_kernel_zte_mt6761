...
bool start_test_ft8607(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_ENTER_FACTORY_MODE) {
		...
		/* FT8607_RAWDATA_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_RAWDATA_TEST) {
		...
		/* FT8607_CB_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_CB_TEST) {
		...
		/* FT8607_SHORT_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_SHORT_CIRCUIT_TEST) {
		...
		/* FT8607_Open_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_OPEN_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8607_LCD_NOISE_TEST) {
...
static int ft8607_lcdnoise_test(bool *test_result)
{
	...
	ret =
		write_reg(REG_8607_LCD_NOISE_FRAME,
			  (unsigned char)(ft8607_basic_thr.lcd_noise_test_frame & 0xFF));
	ret =
		write_reg(REG_8607_LCD_NOISE_FRAME + 1,
			  (unsigned char)((ft8607_basic_thr.lcd_noise_test_frame >> 8) & 0xFF));
...
LCD_NOISE_CHECK:
	...
	ret =
		read_mass_data(FACTORY_REG_RAWDATA_ADDR,
	...
	if (test_data.screen_param.selected_ic == IC_FT8607U) {
		...
		FTS_TEST_SAVE_INFO("\nVA_Set_Range=(%d, %d). Key_Set_Range=(%d, %d). ", value_min, value_max,
				   value_min, value_max_vk);
		result_flag =
			compare_data_incell(lcdnoise, value_min, value_max, value_min, value_max_vk, true);
	}
	/* Save Test Data */
