...
#if (FTS_CHIP_TEST_TYPE == FT5452_TEST)
/*****************************************************************************
...
static int read_rawdata(u8 freq, u8 line_num, int byte_num, int *rev_buffer)
{
	...
	for (i = 1; i < read_num; i++) {
		...
		if (i == read_num - 1) { /* last packet &*/
			ret = fts_i2c_read_write(NULL, 0, read_data +
					BYTES_PER_TIME * i, byte_num - BYTES_PER_TIME * i);
		} else {
...
static void save_testdata_mcap(int *data, char *test_num, int row_array_index, int col_array_index, u8 row, u8 col,
			       u8 item_count)
{
	int len = 0;
	int i = 0, j = 0;
	...
	len = snprintf(test_data.tmp_buffer, BUFF_LEN_TMP_BUFFER, "%s, %d, %d, %d, %d, %d, ",
	...
	for (i = 0 + row_array_index; (i < row + row_array_index)  && (i < TX_NUM_MAX); i++) {
		for (j = 0 + col_array_index; (j < col + col_array_index) && (j < RX_NUM_MAX); j++) {
			if (j == (col - 1))
				...
				len = snprintf(test_data.tmp_buffer, BUFF_LEN_TMP_BUFFER,
						"%d, ", data[i * (col + row_array_index) + j]);
...
static bool compare_scap_data(int *scap_data, u8 line_num, int test_type_0, int test_type_1,
			      int (*data_min)[RX_NUM_MAX], int (*data_max)[RX_NUM_MAX])
{
	int i;
	int min;
	int max;
	...
	if (error_num > 1) {
		...
		for (i = 0; i < error_num; i++) {
			for (j = i + 1; j < error_num; j++) {
				...
				fvalue = (tmp_adc_data[adc_count] - offset + 395) *
					res_stalls / (code_1 - tmp_adc_data[adc_count]) - 3;
				if (fvalue < 0)
					fvalue = abs(fvalue);
				if (min_cc > fvalue) {
	...
	if (min_70k_num > 0) {
		...
		for (i = 0; i < min_70k_num; i++) {
			for (j = i + 1; j < min_70k_num; j++) {
				adc_count++;
				if (code_0 - tmp_adc_data[adc_count] <= 0) {
					fvalue = min_cc;
					continue;
				}
				fvalue = (tmp_adc_data[adc_count] - offset + 395) *
					res_stalls0 / (code_0 - tmp_adc_data[adc_count]) - 3;
				if (fvalue < 0)
					fvalue = abs(fvalue);
		...
		for (row = 0; row < tx_num; row++) {
			for (col = 0; col < rx_num; col++) {
				...
				if (value < min || value > max) {
					...
					fts_save_failed_node(tx_num, rx_num, row, col);
					FTS_TEST_SAVE_ERR
		("Tx Linearity Out Of Range. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d)\n",
		...
		for (row = 0; row < tx_num; row++) { /* row = 1 */
			for (col = 0; col < rx_num; col++) {
				...
				if (value < min || value > max) {
					...
					fts_save_failed_node(tx_num, rx_num, row, col);
					FTS_TEST_SAVE_ERR
	("rx linearity out of range. node=(%d,  %d), get_value=%d, set_range=(%d, %d)\n",
						row + 1, col + 1, value, min, max);
				}
			}
		}
	...
	if (ft5452_basic_thr.uniformity_check_min_max) {
		...
		for (row = 0; row < tx_num; ++row) {
			for (col = 0; col < rx_num; ++col) {
				...
				if (test_data.mcap_detail_thr.invalid_node[row][col] == 2) {
					continue;
				}
...
static int ft5452_panel_differ_test(bool *test_result)
{
	...
	avg_value = avg_value / (tx_num * rx_num - invalid_num);
...
static bool start_test_ft5452(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (item_code == CODE_FT5452_ENTER_FACTORY_MODE) {
		...
		/* FT5452_RAWDATA_TEST */
		if (item_code == CODE_FT5452_RAWDATA_TEST) {
		...
		if (item_code == CODE_FT5452_UNIFORMITY_TEST) {
			ret = ft5452_uniformity_test(&temp_result);
			if ((ret != 0) || (!temp_result)) {
				test_result = false;
				test_data.test_item[item_count].testresult = RESULT_NG;
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_UNIFORMITY_LIMIT;
		...
		/* FT5452_SCAP_CB_TEST */
		if (item_code == CODE_FT5452_SCAP_CB_TEST) {
		...
		if (item_code == CODE_FT5452_SCAP_RAWDATA_TEST) {
		...
		if (item_code == CODE_FT5452_PANELDIFFER_TEST) {
		...
		if (item_code == CODE_FT5452_WEAK_SHORT_CIRCUIT_TEST) {
