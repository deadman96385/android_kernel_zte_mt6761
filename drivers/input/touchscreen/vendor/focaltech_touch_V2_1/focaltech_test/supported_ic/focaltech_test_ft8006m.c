...
bool start_test_ft8006m(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_ENTER_FACTORY_MODE) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_RAWDATA_TEST) {
		...
		/* FT8006m_CB_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_CB_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_SHORT_CIRCUIT_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_LCD_NOISE_TEST) {
		...
		/* FT8006m_OPEN_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8006M_OPEN_TEST) {
...
static int ft8006m_cb_test(bool *test_result)
{
	...
	/* ---------------------------------o Determine RawData if in Range or not */
	tmp_result =
