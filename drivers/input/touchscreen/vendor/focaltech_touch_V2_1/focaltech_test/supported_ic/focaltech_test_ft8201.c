...
static void integrate_data(struct ft8201_info *info, int *m_buf, int *s_buf, int *data)
{
	...
	if (info->m_s_sel.bits.s0_as_slave == false) {
		s0_buf = m_buf;
		s0_tx = info->m_tx;
		s0_rx = info->m_rx;
...
static int check_ic_info_validity(struct ft8201_info *info)
{
	...
	/* I2C addr */
	if ((info->m_i2c_addr == 0) || (info->s_i2c_addr == 0)) {
	...
	/* tx/rx */
	if ((info->m_tx == 0) || (info->m_tx > TX_NUM_MAX)) {
		FTS_TEST_SAVE_ERR("master tx(%d) fail\n", info->m_tx);
		return -EINVAL;
	}

	if ((info->m_rx == 0) || (info->m_rx > TX_NUM_MAX)) {
		FTS_TEST_SAVE_ERR("master rx(%d) fail\n", info->m_rx);
		return -EINVAL;
	}

	if ((info->s_tx == 0) || (info->s_tx > TX_NUM_MAX)) {
		FTS_TEST_SAVE_ERR("slave tx(%d) fail\n", info->s_tx);
		return -EINVAL;
	}

	if ((info->s_rx == 0) || (info->s_rx > TX_NUM_MAX)) {
...
static int ft8201_rawdata_test(struct ft8201_info *info, bool *test_result)
{
	...
	tmp_result = compare_detailthreshold_data_incell(rawdata, test_data.incell_detail_thr.rawdata_test_min,
...
static bool start_test_ft8201(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		/* ENTER_FACTORY_MODE */
		if (item_code == CODE_FT8201_ENTER_FACTORY_MODE) {
		...
		/* SHORT_CIRCUIT_TEST */
		if (item_code == CODE_FT8201_SHORT_CIRCUIT_TEST) {
		...
		/* OPEN_TEST */
		if (item_code == CODE_FT8201_OPEN_TEST) {
		...
		/* CB_TEST */
		if (item_code == CODE_FT8201_CB_TEST) {
		...
		/* RAWDATA_TEST */
		if (item_code == CODE_FT8201_RAWDATA_TEST) {
		...
		/* LCD_NOISE_TEST */
		if (item_code == CODE_FT8201_LCD_NOISE_TEST) {
			ret = ft8201_lcdnoise_test(&info, &tmp_result);
			if ((ret != 0) || (!tmp_result)) {
				test_result = false;
				test_data.test_item[item_count].testresult = RESULT_NG;
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_JITTER_LIMIT;
