...
static int ft8006u_short_test(bool *test_result)
{
	...
	if (ret) {
		FTS_TEST_SAVE_ERR("Failed to get AdcData,ret=%d", ret);
		goto TEST_END;
	}
...
static bool start_test_ft8006u(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (item_code == CODE_FT8006U_ENTER_FACTORY_MODE) {
		...
		/* FT8006u_SHORT_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8006U_SHORT_CIRCUIT_TEST) {
		...
		/* FT8006u_Open_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8006U_OPEN_TEST) {
		...
		/* FT8006u_CB_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FT8006U_CB_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006U_LCD_NOISE_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8006U_RAWDATA_TEST) {
