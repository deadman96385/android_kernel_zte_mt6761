...
bool start_test_ft8606(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8606_ENTER_FACTORY_MODE) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8606_RAWDATA_TEST) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FT8606_CB_TEST) {
...
static int ft8606_cb_test(bool *test_result)
{
	...
	tmp_result =
		compare_detailthreshold_data_incell(cbdata, test_data.incell_detail_thr.cb_test_min,
				test_data.incell_detail_thr.cb_test_max, true);
