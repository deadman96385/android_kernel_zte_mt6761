...
bool start_test_ft3d47(void)
{
	...
	for (iItemCount = 0; iItemCount < test_data.test_num; iItemCount++) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_ENTER_FACTORY_MODE) {
		...
		/* FT3D47_RAWDATA_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_RAWDATA_TEST) {
		...
		/* FT3D47_SCAP_CB_TEST */
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_SCAP_CB_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_SCAP_RAWDATA_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_FORCE_TOUCH_SCAP_CB_TEST) {
		...
		if (test_data.test_item[iItemCount].itemcode == Code_FT3D47_FORCE_TOUCH_SCAP_RAWDATA_TEST) {
...
unsigned char FT3D47_TestItem_ForceTouch_SCapRawDataTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		/* Waterproof rawdata */
		ByteNum =
			(test_data.screen_param.rx_num + test_data.screen_param.tx_num + m_iForceTouchRx +
			 m_iForceTouchTx) * 2;
		ReCode = ReadRawData(0, 0xAC, ByteNum, m_iTempRawData);
		if (ReCode != ERROR_CODE_OK) {
			FTS_TEST_ERROR("Failed to ReadRawData water! ");
		...
		for (m = 0; m < m_iForceTouchTx; m++) {
			m_Force_RawData_Water[0][m] =
				m_iTempRawData[test_data.screen_param.rx_num + test_data.screen_param.tx_num + m];
		}
...
unsigned char FT3D47_TestItem_ForceTouch_SCapCbTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		for (i = 0; i < m_iForceTouchTx; i++) {
			...
			FTS_TEST_DBG("i[%d], CbData[%d]", i,
				     m_RawData[0 + test_data.screen_param.tx_num][m_iForceTouchRx + i]);
			index++;
		}
	...
	test_data.testresult_len +=
		snprintf(test_data.testresult + test_data.testresult_len,
			PAGE_SIZE, "Force Touch SCap CB Test is %s.\n\n",
			 (btmpresult ? "OK" : "NG"));
...
void init_basicthreshold_ft3d47(char *strIniFile)
{
	...
	snprintf(g_stCfg_FT3D47_BasicThreshold.Project_Code,
		 sizeof(g_stCfg_FT3D47_BasicThreshold.Project_Code), "%s", str);
