...
int ft8716_rawdata_test(bool *test_result)
{
	...
	temp_result =
		compare_detailthreshold_data_incell(rawbuf, test_data.incell_detail_thr.rawdata_test_min,
				test_data.incell_detail_thr.rawdata_test_max, key_support);
...
int ft8716_cb_test(bool *test_result)
{
	...
	for (chy = 0; chy < key_num; chy++) {
		if (key_cb_width != 0) {
			cb_buf[tmptx + chy / 2] =
				(short)((tmpbuf[tmptx + chy] & 0x01) << 8) + tmpbuf[tmptx + chy + 1];
			chy++;
		} else {
			cb_buf[tmptx + chy] = tmpbuf[tmptx + chy];
		}
	...
	temp_result =
		compare_detailthreshold_data_incell(cb_buf, test_data.incell_detail_thr.cb_test_min,
				test_data.incell_detail_thr.cb_test_max, key_support);
...
int ft8716_lcd_noise_test(bool *test_result)
{
	...
	for (chx = 0; chx < tx_num + 1; chx++) {
		tmptx = chx * rx_num;
		for (chy = 0; chy < rx_num; chy++) {
			if (ft8716_basicthreshold.lcd_noise_test_mode == 1)
...
bool start_test_ft8716(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (item_code == CODE_FT8716_ENTER_FACTORY_MODE) {
		...
		/* FT8716_RAWDATA_TEST */
		if (item_code == CODE_FT8716_RAWDATA_TEST) {
		...
		/* FT8716_CB_TEST */
		if (item_code == CODE_FT8716_CB_TEST) {
		...
		if (item_code == CODE_FT8716_SHORT_CIRCUIT_TEST) {
		...
		/* FT8716_Open_TEST */
		if (item_code == CODE_FT8716_OPEN_TEST) {
		...
		if (item_code == CODE_FT8716_LCD_NOISE_TEST) {
		...
		if (item_code == CODE_FT8716U_KEY_SHORT_TEST) {
