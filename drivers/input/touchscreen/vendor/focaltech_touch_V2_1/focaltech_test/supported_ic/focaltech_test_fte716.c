...
bool start_test_fte716(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (test_data.test_item[item_count].itemcode == CODE_FTE716_ENTER_FACTORY_MODE) {
			ret = fte716_enter_factory_mode();
			if (ret != ERROR_CODE_OK || (!tmp_result)) {
				test_result = false;
				test_data.test_item[item_count].testresult = RESULT_NG;
				goto ITEM_FAILE;	/* if this item FAIL, no longer test. */
			} else
				test_data.test_item[item_count].testresult = RESULT_PASS;
		}
		/* FTE716_RAWDATA_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FTE716_RAWDATA_TEST) {
		...
		/* FTE716_CB_TEST */
		if (test_data.test_item[item_count].itemcode == CODE_FTE716_CB_TEST) {
...
static int fte716_cb_test(bool *test_result)
{
	...
	ret =
		get_cb_incell(0, test_data.screen_param.tx_num * test_data.screen_param.rx_num + read_key_len,
			      cbdata);
	if (ret != ERROR_CODE_OK) {
		tmp_result = false;
