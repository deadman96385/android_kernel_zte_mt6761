										...
										size_t len, loff_t *pos)
{
	...
	if (ts->debug_log_level & BIT(3)) {
		if (ts->pdata->screenWidth > 0 && ts->pdata->screenHeight > 0 &&
			...
			ts->widthFactor =
				(ts->pdata->screenWidth << SHIFTBITS) / (ts->pdata->abs_x_max - ts->pdata->abs_x_min);
			ts->heightFactor =
				(ts->pdata->screenHeight << SHIFTBITS) / (ts->pdata->abs_y_max - ts->pdata->abs_y_min);
...
int himax_set_diag_cmd(struct himax_ic_data *ic_data, struct himax_report_data *hx_touch_data)
{
	...
	if (hx_touch_data->diag_cmd >= 1 && hx_touch_data->diag_cmd <= 7) {
		...
		g_core_fp.fp_diag_parse_raw_data(hx_touch_data, mul_num, self_num,
...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd == 0) {
		...
		memcpy(buf, hx_touch_data->hx_coord_buf, hx_touch_data->touch_info_size);
		memcpy(&buf[hx_touch_data->touch_info_size], hx_touch_data->hx_rawdata_buf,
...
void himax_log_touch_event(struct himax_ts_data *ts, int start)
{
	...
	if (g_target_report_data->finger_on > 0 && g_target_report_data->finger_num > 0) {
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			if (g_target_report_data->x[loop_i] >= 0 && g_target_report_data->x[loop_i] <=
				ts->pdata->abs_x_max && g_target_report_data->y[loop_i] >= 0 &&
			...
			if (((ts->old_finger >> loop_i & 1) == 0) && ((ts->pre_finger_mask >> loop_i & 1) == 1)) {
				if (g_target_report_data->x[loop_i] >= 0 && g_target_report_data->x[loop_i] <=
					...
					I("status: Raw:F:%02d Down, X:%d, Y:%d, W:%d\n", loop_i + 1,
						g_target_report_data->x[loop_i], g_target_report_data->y[loop_i],
						g_target_report_data->w[loop_i]);
				}
...
static int himax_diag_seq_read(struct seq_file *s, void *v)
{
	...
	size_t ret = 0;
	uint32_t loop_i = 0;
	uint16_t mutual_num = 0, self_num = 0, width = 0;
	int dsram_type = 0;
	...
	for (i = 0; i < y_channel; i++) { /*mutual data*/
		for (j = 0; j < x_channel; j++) {
			...
			if (dsram_type == 1 || dsram_type == 4) {
				...
				mutual_data[i * x_channel + j] = mutual_data_new[i * x_channel + j] -
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'h') { /* handshaking */
		...
		I("NOW debug echo r!\n");
		/* himax_program_sram(); */
		private_ts->himax_0f_update_wq = create_singlethread_workqueue("HMX_update_0f_request_write");
									 ...
									 size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		for (loop_i = 0; loop_i < 1024; loop_i++) {
			ret += snprintf(temp_buf + ret, len - ret, "x%2.2X",
