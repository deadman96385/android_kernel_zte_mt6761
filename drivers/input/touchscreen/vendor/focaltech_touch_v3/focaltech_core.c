...
#ifdef FTS_REST_DELAYED_TIME
static void fts_reset_func(struct work_struct *work)
{
#ifdef FTS_ESDCHECK_REGISTER
	fts_data->esd_register_check_enable = false;
	fts_reset_proc(200);
	fts_data->esd_register_check_enable = true;
#else
	fts_reset_proc(200);
#endif
}
...
#ifdef FTS_REST_DELAYED_TIME
	if (ts_data->ts_workqueue) {
		INIT_DELAYED_WORK(&ts_data->boot_delayed_reset, fts_reset_func);
		queue_delayed_work(ts_data->ts_workqueue, &ts_data->boot_delayed_reset,
			msecs_to_jiffies(FTS_REST_DELAYED_TIME));
	}
#endif
#if defined(CONFIG_ADF) || defined(CONFIG_FB)
