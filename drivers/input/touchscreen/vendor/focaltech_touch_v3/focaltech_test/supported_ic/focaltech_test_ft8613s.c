...
*
* File Name: Focaltech_test_ft8613s.c
*
...
#if (FTS_CHIP_TEST_TYPE == FT8613_TEST)
/*****************************************************************************
...
*****************************************************************************/
static int ft8613s_short_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < tdata->node.node_num; i++) {
		...
		adcdata[i] = (20150 + 67 * tmp_adc) / (4030 - tmp_adc);
	}
...
static int ft8613s_open_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		if ((ret >= 0) && (state == FACTORY_REG_OPEN_IDLE)) {
			break;
		} else if ((ret >= 0) && (state == FACTORY_REG_OPEN_BUSY)) {
			sys_delay(FACTORY_TEST_RETRY_DELAY);
		} else if ((ret >= 0) && (state == TEST_RETVAL_AA)) {
			ret = fts_test_write_reg(FACTORY_REG_OPEN_STATE, FACTORY_REG_OPEN_IDLE);
			if (ret < 0) {
				FTS_TEST_SAVE_ERR("write reg 0x16 fail\n");
				goto test_err;
			}
			break;
		}
...
static int ft8613s_cb_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	int byte_num = 0;
...
static int ft8613s_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	int *rawdata = NULL;
...
static int ft8613s_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
...
test_err:
	...
	/*save test data */
	fts_test_save_data("LCD Noise Test", CODE2_LCD_NOISE_TEST, lcdnoise, 0, false, false, *test_result);
	FTS_TEST_FUNC_EXIT();
	return ret;
}

static int start_test_ft8613s(void)
{
	int ret = 0;
	...
	if (test_item->short_test == true) {
		ret = ft8613s_short_test(tdata, &temp_result);
	...
	if (test_item->open_test == true) {
		ret = ft8613s_open_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
	...
	if (test_item->cb_test == true) {
		ret = ft8613s_cb_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
	...
	if (test_item->rawdata_test == true) {
		ret = ft8613s_rawdata_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
	...
	if (test_item->lcdnoise_test == true) {
		ret = ft8613s_lcdnoise_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
...
struct test_funcs test_func_ft8613s = {
	.ctype = {0x14},
	.hwtype = IC_HW_INCELL,
	.key_num_total = 0,
	.start_test = start_test_ft8613s,
};
#endif
