...
*****************************************************************************/
static int ft8739_short_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < tdata->node.node_num; i++) {
		...
		adcdata[i] = 184320 / (tmp_adc - 2048) - 60;
	}
...
static int ft8739_open_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	int byte_num = 0;
...
static int ft8739_cb_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
...
static int ft8739_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
...
static int ft8739_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	int i = 0;
	bool tmp_result = false;
...
static int start_test_ft8739(void)
{
	int ret = 0;
	...
	if (test_item->short_test == true) {
		ret = ft8739_short_test(tdata, &temp_result);
	...
	if (test_item->open_test == true) {
		ret = ft8739_open_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
	...
	if (test_item->cb_test == true) {
		ret = ft8739_cb_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
	...
	if (test_item->rawdata_test == true) {
		ret = ft8739_rawdata_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
	...
	if (test_item->lcdnoise_test == true) {
		ret = ft8739_lcdnoise_test(tdata, &temp_result);
		if ((ret < 0) || (temp_result == false)) {
			test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
