...
#if (FTS_CHIP_TEST_TYPE == FT6426_TEST)
/*****************************************************************************
...
static void init_dcb_ft6426(int *delta_cb, struct sc_threshold *thr, struct dcb_sort_d *sort)
{
	int i = 0;
...
static int ft6426_delta_cb_test(struct fts_test *tdata, bool *test_result)
{
	...
	/* get differ */
	cb_2 = fts_malloc(tdata->node.node_num * sizeof(int));
	if (cb_2 == NULL) {
	...
	for (i = 0; i < tdata->node.node_num; i++) {
		delta_cb[i] = focal_abs(cb_2[i] - cb_1[i] - thr->dcb_base[i]);
	}
...
static int start_test_ft6426(void)
{
	...
	tmpcb = fts_malloc(tdata->node.node_num * sizeof(int));
	if (tmpcb == NULL) {
