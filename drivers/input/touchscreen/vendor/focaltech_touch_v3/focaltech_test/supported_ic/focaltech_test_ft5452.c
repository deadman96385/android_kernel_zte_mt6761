...
static bool short_test_get_channel_num(struct fts_test *tdata, int *fm_short_resistance, int *adc_data, int offset,
				       bool *is_weak_short_mut)
{
	int ret = 0;
	int i;
	int count = 0;
...
static bool short_test_channel_to_gnd(struct fts_test *tdata, int *fm_short_resistance, int *adc_data, int offset,
				      bool *is_weak_short_gnd)
{
	int ret = 0;
	int error_num = 0;
	int min_70k_num = 0;
	...
	if (min_70k_num > 0) {
		...
		for (i = 0; i < 1; i++) {
			ret =
			    short_get_adc_data_mc(TEST_RETVAL_00, min_70k_num * 2, tmp_adc_data, FACTROY_REG_SHORT_CG);
			sys_delay(50);
			if (ret) {
				FTS_TEST_SAVE_ERR("failed to get weak short data,ret= %d\n", ret);
				tmp_result = false;
...
static bool short_test_channel_to_channel(struct fts_test *tdata, int *fm_short_resistance, int *adc_data, int offset)
{
	...
	if (error_num > 1) {
		...
		for (j = 0; j < 1; j++) {
			ret =
			    short_get_adc_data_mc(TEST_RETVAL_00, error_num * (error_num - 1) * 2 / 2, tmp_adc_data,
						  FACTROY_REG_SHORT_CC);
			sys_delay(50);
			if (ret) {
				FTS_TEST_SAVE_ERR("failed to get weak short data,ret= %d\n", ret);
				tmp_result = false;
		...
		for (i = 0; i < error_num; i++) {
			for (j = i + 1; j < error_num; j++) {
				...
				fvalue =
				    (tmp_adc_data[adc_count] - offset + 395) * res_stalls / (code_1 -
					 tmp_adc_data[adc_count]) - 3;
				adc_count++;
				if (fvalue < 0)
					fvalue = abs(fvalue);
				if (min_cc > fvalue) {
	...
	if (min_70k_num > 0) {
		...
		for (i = 0; i < 1; i++) {
			ret =
			    short_get_adc_data_mc(TEST_RETVAL_00, min_70k_num * (min_70k_num - 1) * 2 / 2, tmp_adc_data,
						  FACTROY_REG_SHORT_CC);
			sys_delay(50);
			if (ret) {
				FTS_TEST_SAVE_ERR("failed to get weak short data,ret= %d\n", ret);
		...
		for (i = 0; i < min_70k_num; i++) {
			for (j = i + 1; j < min_70k_num; j++) {
				...
				fvalue =
				    (tmp_adc_data[adc_count] - offset + 395) * res_stalls0 / (code_0 -
						   tmp_adc_data[adc_count]) - 3;
				adc_count++;
				if (fvalue < 0)
					fvalue = abs(fvalue);
...
static int ft5452_uniformity_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (!thr->tx_linearity_max || !thr->rx_linearity_max || !tdata->node_valid || !test_result) {
...
test_err:
	...
	fts_test_save_data("Rawdata Uniformity Test",
			   CODE_M_RAWDATA_UNIFORMITY_TEST, rawdata_linearity, 0, false, false, *test_result);
...
static int ft5452_short_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (is_weak_short_mut) {
		tmp_result =
		    short_test_channel_to_gnd(tdata, fm_short_resistance, adc_data, offset, &is_weak_short_gnd);
	}
...
static int start_test_ft5452(void)
{
	...
	if (test_item->rawdata_uniformity_test  == true) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_UNIFORMITY_LIMIT;
		}
	}
