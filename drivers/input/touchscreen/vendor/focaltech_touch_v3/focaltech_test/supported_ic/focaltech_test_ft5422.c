...
#if (FTS_CHIP_TEST_TYPE == FT5422_TEST)
/*****************************************************************************
...
*****************************************************************************/
static int short_ch_gnd_ft5422(struct fts_test *tdata, u8 param, int *adc, int *resistor, bool *result)
{
	int i = 0;
...
static int short_ch_ch_ft5422(struct fts_test *tdata, u8 param, int *adc, int *resistor, bool *result)
{
	int i = 0;
	bool tmp_result = true;
	int dsen = 0;
...
static int ft5422_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (tdata->normalize == NORMALIZE_AUTO) {
		...
		/* compare */
		result = compare_array(rawdata_tmp, thr->rawdata_h_min, thr->rawdata_h_max, false);
		...
		if (thr->basic.rawdata_set_lfreq) {
			...
			/* compare */
			result = compare_array(rawdata_tmp, thr->rawdata_l_min, thr->rawdata_l_max, false);
		...
		if (thr->basic.rawdata_set_hfreq) {
			...
			/* compare */
			result2 = compare_array(rawdata_tmp, thr->rawdata_h_min, thr->rawdata_h_max, false);
...
test_err:
	...
	/* save data */
	fts_test_save_data("Rawdata Test", CODE_M_RAWDATA_TEST, rawdata, rawdata_cnt, false, false, *test_result);
...
static int ft5422_short_test(struct fts_test *tdata, bool *test_result)
{
	...
	FTS_TEST_DBG("get rawdata 3 time");
	for (i = 0; i < 3; i++) {
	...
	if (adc_num > tdata->buffer_length) {
		FTS_TEST_SAVE_ERR("adc num(%d) > buffer len(%d)", adc_num, tdata->buffer_length);
		ret = -EIO;
		goto test_err;
	}
	...
	/* get adc data */
	for (i = 0; i < 5; i++) {
		ret = short_get_adc_data_mc(TEST_RETVAL_00, adc_num * 2, adc, FACTROY_REG_SHORT_CA);
	...
	resistor = fts_malloc(tdata->sc_node.channel_num * sizeof(int));
	if (resistor == NULL) {
	...
	/* channel to channel */
	ret = short_ch_ch_ft5422(tdata, param, &adc[1 + 1 + tdata->sc_node.channel_num], resistor, &tmp2_result);
	if (ret < 0) {
