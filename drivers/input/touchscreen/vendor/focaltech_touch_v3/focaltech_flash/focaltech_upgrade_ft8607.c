...
static int find_bank(u8 *initcode, u16 bank_start_addr, u8 *bank_sign, u16 *bank_pos)
{
	...
	while (pos < initcode_len) {
		if ((initcode[pos] == bank_sign[0])
			...
			FTS_INFO("bank(%x %x %x %x) find", bank_sign[0], bank_sign[1], bank_sign[2], bank_sign[3]);
			*bank_pos = pos;
			return 0;
		}
