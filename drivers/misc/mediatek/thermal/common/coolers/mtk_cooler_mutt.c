...
#ifdef FEATURE_THERMAL_CLEAR
	if (!strncmp(cdev->type, "mtk-cl-mutt00", sizeof("mtk-cl-mutt00") - 1) &&
            state != tmc_state[0]) {
		tmc_state[0] = state;
		clmutt_send_tmc_signal(TMC_level_1 - state);
	}
	if (!strncmp(cdev->type, "mtk-cl-mutt01", sizeof("mtk-cl-mutt01") - 1) &&
            state != tmc_state[1]) {
		tmc_state[1] = state;
		clmutt_send_tmc_signal(TMC_level_2 - state);
	}
	if (!strncmp(cdev->type, "mtk-cl-mutt02", sizeof("mtk-cl-mutt02") - 1) &&
            state != tmc_state[2]) {
		tmc_state[2] = state;
