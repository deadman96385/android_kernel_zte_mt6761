...
static DEFINE_SPINLOCK(imgsensor_drv_lock);
	...
	.sensor_id = SP5508_Z6201V_094_SENSOR_ID,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("superpix Read sensor id fail, id: 0x%x--%d\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
UINT32 SP5508_Z6201V_094_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
