...
struct gc8034_otp_t gc8034_otp_info;
...
void Gc8034_Send_AF_OTP(u8 *data, u32 size)
{
	int i = 0;
	kal_uint32 af_otp_info = 0;

	LOG_INF("GC8034_OTP_AF:infitiy=0x%x\n", gc8034_otp_info.af_infinity);
	LOG_INF("GC8034_OTP_AF:macro=0x%x\n", gc8034_otp_info.af_macro);
	af_otp_info |= gc8034_otp_info.af_infinity << 16;
	af_otp_info |= gc8034_otp_info.af_macro;
	LOG_INF("GC8034_OTP_AF:af_otp_info=0x%x\n", af_otp_info);
	for (i = 0; i < size; i++) {
		*(data+i) = (af_otp_info >> (8 * i)) & 0xFF;
	}
}
			...
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					...
					gc8034_otp_info.dd_param[cnt].y =
						((kal_uint16)ddtempbuff[4 * i + 2] << 4) +
						((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034_otp_info.dd_param[cnt++].t =
	...
	if (gc8034_otp_info.dd_flag == 0x01) {
		...
		for (i = 0; i < gc8034_otp_info.dd_cnt; i++) {
			...
			LOG_INF("GC8034_OTP_GC x = %d , y = %d\n", ((temp_val1&0x0f)<<8) +
				temp_val0, (temp_val2<<4) + ((temp_val1&0xf0)>>4));
		}
