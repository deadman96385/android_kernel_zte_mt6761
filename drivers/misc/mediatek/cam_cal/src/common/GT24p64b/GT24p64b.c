...
static bool selective_read_byte(u32 addr, u8 *data, u16 i2c_id)
{
	...
	CAM_CALERR("ok selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x",
			 addr, *data, page, offset);
...
/*#define GT24P64B_DRIVER_ON 0*/
#ifdef GT24P64B_DRIVER_ON
#ifdef CONFIG_COMPAT
...
static long gt24p64b_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret = 0, err = 0;
	struct COMPAT_stCAM_CAL_INFO_STRUCT *data32 = NULL;
	struct stCAM_CAL_INFO_STRUCT __user *data = NULL;

	CAM_CALDB("[CAMERA SENSOR] gt24p64b_Ioctl_Compat,%p %p %x ioc size %d\n",
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/* .ioctl = CAM_CAL_Ioctl */
#ifdef CONFIG_COMPAT
	.compat_ioctl = gt24p64b_Ioctl_Compat,
#endif
